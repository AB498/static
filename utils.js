...
document.body.insertAdjacentElement(
  ...
  new DOMParser().parseFromString(
    `<div
        ...
        <div id="debug-settings" class="flex flex-col hidden bg-zinc-50 dark:bg-zinc-950/75">
          <label for="debug-auto-open" class="flex gap-2 p-2">
...
window.debug = (...args) => {
  ...
  if (typeof args[0] == "object") {
    let jsv = new DOMParser()
      .parseFromString(
        `<div class="m-1 border-2 border-green-600/50 rounded-md p-1 flex flex-col bg-zinc-50 dark:bg-zinc-950/75 pt-1 font-mono whitespace-pre-wrap"><div class="time bg-zinc-50 dark:bg-zinc-950/75 rounded-md p-1 text-xs class="w-full overflow-x-auto"">${time}</div></div>`,
        "text/html"
      )
    ...
    let jsv = new DOMParser()
      .parseFromString(
        `<div class="m-1 border-2 border-green-600/50 rounded-md p-1 flex flex-col bg-zinc-50 dark:bg-zinc-950/75 pt-1 font-mono whitespace-pre-wrap"><div class="time bg-zinc-50 dark:bg-zinc-950/75 rounded-md p-1 text-xs">${time}</div><div class="w-full overflow-x-auto">${argsString}</div></div>`,
        "text/html"
      )
      .documentElement.querySelector("body").firstChild;
    document.querySelector(".debug").insertAdjacentElement("afterbegin", jsv);
...
window.error = (...args) => {
  ...
  if (typeof args[0] == "object") {
    let jsv = new DOMParser()
      .parseFromString(
        `<div class="m-1 border-2 border-green-600/50 rounded-md p-1 flex flex-col bg-red-500 rounded-md p-1 text-xs class="w-full overflow-x-auto"">${time}</div></div>`,
        "text/html"
      )
      .documentElement.querySelector("body").firstChild;
    let jsv2 = new DOMParser()
    ...
    let jsv = new DOMParser()
      .parseFromString(
        `<div class="m-1 border-2 border-green-600/50 rounded-md p-1 flex flex-col bg-red-800 pt-1 font-mono whitespace-pre-wrap"><div class="time bg-zinc-50 dark:bg-zinc-950/75 rounded-md p-1 text-xs">${time}</div><div class="w-full overflow-x-auto">${argsString}</div></div>`,
        "text/html"
      )
      .documentElement.querySelector("body").firstChild;
    document.querySelector(".debug").insertAdjacentElement("afterbegin", jsv);
