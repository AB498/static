  ...
  useEffect(() => {
    (async () => {
      ...
      state.current.previewImage =
        (await safe(async () =>
          URL.createObjectURL(
            (await downImage("previews/" + identity.slug + ".png")) || (await downImage("https://corsproxy.io/?https://oldie.veriftools.ru/media/generators/previews/usa_passport_preview_new.jpg"))
          )
        )) || "no-image.png";
          ...
          {!state.current.currentId ? (
            ...
            <div className="flex flex-col sm:flex-row basis-2/3 gap-6 ">
              ...
              <div className="sm:basis-1/2">
                <div className="full  gap-2 bg-white min-h-[100px] shadow rounded-xl border-2 result flex flex-col p-6">
                  <div className="text-lg font-bold center">{generationProgress.current.status == 'completed' ? 'Result': 'Preview'}</div>
                  <div className="grow"></div>
