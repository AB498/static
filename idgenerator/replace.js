...
const rules = [
  ...
  {
    pattern: "Evaluation Warning : The document was created with Spire.PDF for Python",
    replacement: "",
  },
];
async function isValidBuffer(buffer) {
  try {
    await sharp(buffer).toFile("tmp.png");
    return true;
  } catch (error) {
    return false;
  }
...
const replaceText = async (data) => {
  ...
  for (let [pdfRef, pdfObject] of pdfDoc.context.enumerateIndirectObjects()) {
    ...
    if (pdfObject?.dict?.get(PDFName.of("Subtype")) === PDFName.of("Image")) {
      ...
      //     if (await isValidBuffer(pdfObject.contents)) {
      ...
      //       newBuffer = await sharp(myImgBuffer)
      //         .resize(w, h)
      //         .toBuffer();
    ...
    //   console.log(text.match(rule.pattern), rule.pattern);
...
calling_dir = __dirname;
file = calling_dir + "/src/templates/uk_dl_front.pdf";
file = "mod.pdf";
replaceText(fs.readFileSync(file)).then((buf) => fs.writeFileSync("./modJS.pdf", buf));
