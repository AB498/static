...
function darkModeSwitch(checked) {
  document.querySelector("#theme-toggle") && (document.querySelector("#theme-toggle").checked = checked);
  document.querySelector("html") && document.querySelector("html").classList.toggle("dark", checked);
  localStorage.theme = checked ? "dark" : "light";
  // emit event darkModeSwitch
  window.dispatchEvent(new CustomEvent("darkModeSwitch", { detail: checked }));
}
...
async function fetchAndReload(filename) {
  if (filename == "/" || filename.slice(-5) == ".html") {
...
window.onload = () => {
  ...
  document.body.insertAdjacentElement(
    ...
    new DOMParser().parseFromString(
      `<div
        class="debugcol hover:opacity-75 text-xs items-stretch transition-all duration-300 h-8 w-full dark:text-zinc-50 text-zinc-950/75 bg-zinc-50 dark:bg-zinc-950/75 flex flex-col overflow-auto whitespace-pre-wrap transition-all bottom-0 fixed z-50 backdrop-filter backdrop-blur-sm">
        <div class="flex gap-1 h-8 w-full shrink-0 justify-center font-bold bg-zinc-100 dark:bg-zinc-950/75 p-1 sticky top-0 z-10 border-b shadow">
            <div class="debugtitle grow rounded hover:bg-gray-500  bg-zinc-200 dark:bg-zinc-900/75 flex justify-center items-center "
             onclick="
             if (document.querySelector('.debugcol').classList.contains('h-1/2')) {
               document.querySelector('.debugcol').classList.remove('h-1/2');
               document.querySelector('.debugcol').classList.add('h-8');
             }else{
               document.querySelector('.debugcol').classList.remove('h-8');
               document.querySelector('.debugcol').classList.add('h-1/2');
             }
             "
            > LOGS </div>
...
window.deb_log = (...args) => {
  ...
  let time = (hours % 12 || 12) + ":" + new Date().getMinutes() + ":" + new Date().getSeconds() + "." + new Date().getMilliseconds() + " " + (hours >= 12 ? "PM" : "AM");
  ...
  if (window.localStorage.getItem("debug-auto-open") == "true") document.querySelector(".debugcol").classList.remove("h-8");
};
...
let originalLog = window.console.log.bind(window.console);
...
function f(o) {
  return escapeHTML(safeStringify(o));
}
let overlayEls = [];
